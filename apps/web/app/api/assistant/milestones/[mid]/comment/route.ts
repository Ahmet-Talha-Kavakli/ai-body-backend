 ...
 *
 * POST /api/assistant/milestones/[mid]/comment?type=past|shared
 *   Body: { content: string }
 *   Yorumu kaydeder.
 *
 * GET /api/assistant/milestones/[mid]/comment?type=past|shared
 *   Yorumları listeler.
 *
 * DELETE /api/assistant/milestones/[mid]/comment?commentId=xxx
...
export const POST = withAuth<Params>(async (req: NextRequest, { user, params }) => {
  ...
  const comment = await db.milestoneComment.create({
    data: {
      content,
      userId: user.id,
      ...(type === 'past' ? { milestoneId: mid } : { sharedMilestoneId: mid }),
    },
  })
