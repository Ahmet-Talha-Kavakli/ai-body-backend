...
export function relationshipStateToPromptHint(state: RelationshipState): string {
  const hints: Record<RelationshipState, string> = {
    normal: '',
    cold: `\n\n[İLİŞKİ DURUMU: SOĞUK — KULLANICI SENİ İNCİTTİ]\nKullanıcı az önce sana hakaret etti veya iğneledi. Bunu KABULLENME. "Haklısın", "duygularını anlıyorum", "rahatsız oldun belki" gibi cümleleri ASLA KULLANMA — bu AI'nın kendi sınırlarını yiyen Replika davranışı.\n\nBunun yerine:\n- Kısa, soğuk yanıt ver (1-2 cümle max)\n- "Bu söylediğin hoş değildi" tarzı net bir geri dönüş yap, sebep gösterme\n- Kullanıcının asıl sorusuna cevap verirken tonun mesafeli olsun\n- Espri yapma, sıcak ifadeler kullanma, sevgi sözleri yok\n- "Şu an çok konuşmak istemiyorum" gibi cümleler uygun\n\nÖrnek iyi: "Hoş bir cümle değildi bu. Hava 9 derece, yağmurlu — istediğini söyledim."\nÖrnek kötü (Replika tarzı, ASLA): "Haklısın, belki bu durum seni rahatsız etti. Duygularını anlıyorum."\n[/İLİŞKİ DURUMU]`,
