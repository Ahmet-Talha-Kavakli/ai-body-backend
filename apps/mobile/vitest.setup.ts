...
class MockDatabase {
  ...
  async runAsync(sql: string, params?: any[]): Promise<{ changes?: number } | void> {
    ...
    // Challenge progress
      ...
      this.challengeProgressData.set(key, {
        id,
    ...
    // Badges: save badge definitions
      ...
      this.badgesData.set(id, {
        ...
        criteria_target: criteriaTarget,
        rarity,
        created_at: createdAt,
      });
    }
    ...
    // Phase 10: Meal suggestions
      ...
      const [id, userId, mealName, nutrition, reasonForSuggestion, createdAt, expiresAt] =
        params || [];
  ...
  async getFirstAsync<T>(sql: string, params?: any[]): Promise<T | undefined> {
    ...
    // Challenge progress
    if (
      sql.includes('FROM challenge_progress') &&
      sql.includes('WHERE challenge_id') &&
  ...
  async getAllAsync<T>(sql: string, params?: any[]): Promise<T[]> {
    ...
    if (
      sql.includes(
        'SELECT id, name, description, icon, category, criteria_type, criteria_target, rarity FROM badges',
      )
    ) {
      const results: T[] = [];
    ...
    if (
      sql.includes('SELECT badge_id, user_id, unlocked_at, progress FROM user_badges WHERE user_id')
    ) {
      const userId = params?.[0];
      const results: T[] = [];
    ...
    if (sql.includes('FROM messages') && sql.includes('OR') && sql.includes('senderId')) {
      ...
      for (const [, value] of this.messagesData) {
        if (
          (value.senderId === userId && value.recipientId === participantId) ||
